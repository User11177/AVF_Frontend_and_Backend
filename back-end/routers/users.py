...
# 新增帳號
@router.post("/users")
def create_user(payload: CreateUserPayload, user=Depends(get_current_user())):
    conn = get_db()
    cursor = conn.cursor()
    try:
        # 新增: 處理密碼
        ...
        cursor.execute("""
            INSERT INTO users (full_name, id_number, email, phone, birthdate, address, emergency_name, emergency_phone, email_verified, phone_verified, role, password_hash)
            VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
        """, (
            payload.full_name, payload.id_number, payload.email, payload.phone,
...
def update_user(user_id: int, payload: UpdateUserPayload, user=Depends(get_current_user())):
    conn = get_db()
    cursor = conn.cursor()
    updates, params = [], []
